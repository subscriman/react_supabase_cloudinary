...
import { getStartAlertExhibitionId } from '../../../lib/start-alerts';
...
export default async function handler(
  ...
  const exhibitionId = getStartAlertExhibitionId(req.query.exhibitionId);
  if (!exhibitionId) {
  ...
  try {
    ...
    if (!deleted) {
      return sendApiError(res, 404, 'NOT_FOUND', '시작 알림 신청 내역을 찾지 못했습니다.');
    }
    ...
    return sendApiError(res, 500, 'INTERNAL_ERROR', mapUnknownError(error, '시작 알림 해제 중 오류가 발생했습니다.'));
  }
}
